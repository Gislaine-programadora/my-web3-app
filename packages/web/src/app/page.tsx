...
import confetti from 'canvas-confetti'; // IMPORTANDO O CONFETE
...
export default function GbitExchangePro() {
  ...
  const { data: gbitBalance, refetch: refetchGbit } = useBalance({ address, token: GBIT_ADDRESS as `0x${string}` });
  const { data: contractEth, refetch: refetchEthContract } = useBalance({ address: GBIT_ADDRESS as `0x${string}` });
  const { data: currentRate } = useReadContract({ address: GBIT_ADDRESS as `0x${string}`, abi: ABI, functionName: 'rate' });

  const { data: hash, writeContract, isPending } = useWriteContract();
  const { isSuccess } = useWaitForTransactionReceipt({ hash });
 
  // Adicione estas novas leituras logo abaixo das que já tens
const { data: totalSupply } = useReadContract({ 
  address: GBIT_ADDRESS as `0x${string}`, 
  abi: [ ...ABI, { name: "totalSupply", type: "function", stateMutability: "view", inputs: [], outputs: [{type:"uint256"}] } ], 
  functionName: 'totalSupply' 
});

// Memo para formatar os dados de forma bonita
const stats = useMemo(() => ({
  total: totalSupply ? parseFloat(formatEther(totalSupply)).toLocaleString() : "0",
  liquidity: contractEth?.value ? parseFloat(formatEther(contractEth.value)).toFixed(4) : "0.0000",
  holders: "1" // Como é testnet, podemos simular ou deixar fixo por enquanto
}), [totalSupply, contractEth]);


  // 1. EFEITO DE VITÓRIA (CONFETE)
  useEffect(() => {
    if (isSuccess) {
      confetti({
        particleCount: 150,
  ...
  const safeGbit = useMemo(() => gbitBalance?.value ? parseFloat(formatEther(gbitBalance.value)).toFixed(2) : "0.00", [gbitBalance]);
  const contractLiquidity = useMemo(() => contractEth?.value ? parseFloat(formatEther(contractEth.value)).toFixed(4) : "0.0000", [contractEth]);
  ...
  // GRÁFICO COM MOVIMENTO REALISTA
  useEffect(() => {
    const generateData = () => {
      const data = [];
  ...
  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
    {/* ADICIONANDO O TEU LOGO DOURADO */}
    <img src="/logo.png" alt="GBIT Logo" style={{ width: '90px', height: '90px', borderRadius: '8px' }} />
